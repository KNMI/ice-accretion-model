<?php
  ...
  if ($INTERFACE == "public")
  {
     ...
     $snwat            = strip_tags($_POST['geselecteerde_sneeuwhoogte']);       // in 1 cm
     $sneeuwleeftijd   = strip_tags($_POST['geselecteerde_sneeuwleeftijd']);
     ...
     $forecast_periode = strip_tags($_POST['geselecteerde_aantal_forecast_uren']);
     $mprec            = 0;  // ?????????????????????????????????/
     ...
     // sneeuwleeftijd van dagen -> 12 uur periodes
     $msnage = Omzetten_Sneeuwleeftijd_Naar_Periode($sneeuwleeftijd, $DEBUG, $file_debug);
  ...
  if ($DEBUG != 0)
  {
        ...
        fwrite($file_debug, "----> geselecteerde_sneeuwhoogte [cm] = " . $snwat . "<br>");
        fwrite($file_debug, "----> geselecteerde_sneeuwleeftijd [aantal 12 uur periodes] = " . $msnage . "<br>");
  ...
  if ($DEBUG != 0)
  {
     ...
     fwrite($file_debug, "+---- snwat = " . $snwat . "<br>");
     fwrite($file_debug, "+---- msnage = " . $msnage . "<br>");
...
///     $sntot = $sntot + $snwat[j];
///  }
  $mzs = round(1000.0 * $snwat / (90.0 + 6.0 * $msnage));  //round(3.4) --> 3 en round(3.5) --> 4
  $sntot = $snwat;
  ...
  for ($jj = 1; $jj <= $mslow; $jj++)   // OF $jj = 0; $jj < $mslow ???????????????????????
  {
     ...
     if (($qzi > 0.0) && (!(($tw == $pptf) && ($mx == 3)))) 
     {
        ...
        {   
           ...
           if ($DEBUG != 0)
           {
              fwrite($file_debug, "+---- snloss[via te > 0] = " . $snloss . "<br>");   
           }
        }
        else
        {       
     ...
     if ($mx == 0) 
     {
     	  ...
     	  if ($DEBUG != 0)
        {
           fwrite($file_debug, "+---- snloss[via ev] = " . $snloss . "<br>");   
        }
     }
     ...
     if (($snwat != 0.0) && ($snloss < 0.0)) 
     {
        ...
        {
           $snloss = $snloss + $snwat;
           $mzs = $mzs - round(1000.0 * $snwat / (90.0 + 6.0 * $msnage));
           $sntot = $sntot - $snwat;
           $snwat = 0.0;
        } // else
     }
...
///     } // if (!(($mhh == 24) || ($qzi < 46.0)))
  ...
  /*                                                                                                           */
  /*************************************************************************************************************/ 
  function Omzetten_Sneeuwleeftijd_Naar_Periode($sneeuwleeftijd, $DEBUG, $file_debug)
  {
  	  if ($sneeuwleeftijd == "nooit")
  	  {
  	  	  $aantal_24_uur_peroden = 9999;
  	  }
  	  else 
  	  {
  	     $aantal_24_uur_peroden = $sneeuwleeftijd * 2;   // dus 3 dagen gelden is 3 * 2 = 6 perioden van 12 uur geleden
  	  }
  	  
     if ($DEBUG != 0)
     {
        fwrite($file_debug, "+---- function Omzetten_Sneeuwleeftijd_Naar_Periode: aantal_24_uur_peroden [aantal] = " . $aantal_24_uur_peroden . "<br>");
        fwrite($file_debug, "<br>");
     }
  	    	  
  	  
  	  return $aantal_24_uur_peroden;
  }
